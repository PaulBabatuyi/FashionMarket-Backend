...
func (m UserModel) GetForClient(userID int64) (*UserWithPermissions, error) {
	query := `
        SELECT 
            u.id, u.email, u.name, u.activated,
	...
	err := m.DB.QueryRowContext(ctx, query, userID).Scan(
		...
		&user.Name,
		&user.Activated,
		pq.Array(&user.Permissions), // pq.Array handles PostgreSQL array → []string
	)
	if err != nil {
...
func ValidatePasswordPlaintext(v *validator.Validator, password string) {
	...
	v.Check(validator.Matches(password, regexp.MustCompile(`[A-Z]`)), "password", "must contain at least one uppercase letter")
	v.Check(validator.Matches(password, regexp.MustCompile(`[0-9]`)), "password", "must contain at least one number")
}
